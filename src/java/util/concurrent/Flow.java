...
/**
 * ========== 此类为Java9中FlowApi实现的3个类之一，另外两个类分别为：Helpers 和SubmissionPublisher。==========
 * ========== 为了方便学习，便将这个3个类Copy到Java8中的源码来。==========
 * 此接口定义了4个角色：
...
public final class Flow {
    ...
    public static interface Subscriber<T> {
         ...
         * 订阅消息，入参是一个Subscription，可以通过Subscription与生产者沟通
         */
        public void onSubscribe(Subscription subscription);
     ...
     * Processor同时兼有发布者和消费者两个角色功能。　通过这两种角色，我们可以在Publisher和Subscriber之间转换数据。
     * 处理器（Processor）是位于最终发布者和订阅者之间的实体，用于转换从发布者接收的数据，
     * 以便订阅者能理解它。我们可以拥有一系列（chain ）处理器。
     *End Publicsher1 ------> | Processor(subscriber)  Processor(Publisher) -------> | Processor(subscriber)  Processor(Publisher) ----->End Subscriber
     * @param <T> the subscribed item type
